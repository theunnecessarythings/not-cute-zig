...
var flash_h64_causal_shared_q: [config.flash_warps * config.flash_block_m * config.mma_k]f16 addrspace(.shared) = undefined;
var flash_h64_causal_shared_k: [config.flash_warps * config.mma_k * config.flash_mma_n]f16 addrspace(.shared) = undefined;
var flash_h64_causal_shared_v: [config.mma_k * config.flash_mma_n]f16 addrspace(.shared) = undefined;
var flash_h64_causal_shared_p: [config.flash_block_m * config.mma_k]f16 addrspace(.shared) = undefined;
var flash_h64_causal_shared_p_mma: [config.flash_block_m * config.mma_k]f16 addrspace(.shared) = undefined;
var flash_h64_causal_scores_shared: [config.flash_block_m * config.flash_block_n]f32 addrspace(.shared) = undefined;
var flash_h64_causal_row_max_shared: [config.flash_block_m]f32 addrspace(.shared) = undefined;
var flash_h64_causal_row_sum_shared: [config.flash_block_m]f32 addrspace(.shared) = undefined;
var flash_h64_causal_old_scale_shared: [config.flash_block_m]f32 addrspace(.shared) = undefined;
var flash_h64_causal_reduce_shared: [64]f32 addrspace(.shared) = undefined;

pub fn flash_attention_fwd_h64_causal(
    ...
    const shared_q_all_ptr: [*]addrspace(.shared) f16 = @ptrCast(&flash_h64_causal_shared_q);
    const shared_k_all_ptr: [*]addrspace(.shared) f16 = @ptrCast(&flash_h64_causal_shared_k);
    const shared_v_ptr: [*]addrspace(.shared) f16 = @ptrCast(&flash_h64_causal_shared_v);
    const shared_p_ptr: [*]addrspace(.shared) f16 = @ptrCast(&flash_h64_causal_shared_p);
    const shared_p_mma_ptr: [*]addrspace(.shared) f16 = @ptrCast(&flash_h64_causal_shared_p_mma);
    ...
    if (thread_id < config.flash_block_m) {
        flash_h64_causal_row_max_shared[thread_id] = -std.math.inf(f32);
        flash_h64_causal_row_sum_shared[thread_id] = 0;
        flash_h64_causal_old_scale_shared[thread_id] = 0;
    }

    blockBarrier();

    const tile_end = @min(seq_len, q_base + config.flash_block_m);
    ...
    while (tile_start < tile_end) : (tile_start += config.flash_block_n) {
        ...
        inline for (0..4) |acc_i| {
            ...
            const q_row = q_base + row;
            scores[acc_i] = if (q_row < seq_len and key_row < seq_len and key_row <= q_row)
                scores[acc_i] * scale
            else
                -std.math.inf(f32);
            flash_h64_causal_scores_shared[row * config.flash_block_n + block_col] = scores[acc_i];
        }
        ...
        if (thread_id < config.flash_block_m * 4) {
            ...
            inline for (0..4) |i| {
                ...
                partial_max = @max(partial_max, flash_h64_causal_scores_shared[row * config.flash_block_n + col]);
            }
            flash_h64_causal_reduce_shared[thread_id] = partial_max;
        }
        ...
        if (thread_id < config.flash_block_m) {
            const row = thread_id;
            const tile_max = @max(@max(flash_h64_causal_reduce_shared[row * 4], flash_h64_causal_reduce_shared[row * 4 + 1]), @max(flash_h64_causal_reduce_shared[row * 4 + 2], flash_h64_causal_reduce_shared[row * 4 + 3]));
            const old_m = flash_h64_causal_row_max_shared[row];
            const old_l = flash_h64_causal_row_sum_shared[row];
            const m_new = @max(old_m, tile_max);
            const old_scale = if (old_l == 0) 0 else device.ex2_approx((old_m - m_new) * log2e);
            flash_h64_causal_row_max_shared[row] = m_new;
            flash_h64_causal_old_scale_shared[row] = old_scale;
        }
        ...
        if (thread_id < config.flash_block_m * 4) {
            const row = thread_id / 4;
            const quarter = thread_id % 4;
            const m_new = flash_h64_causal_row_max_shared[row];
            ...
            inline for (0..4) |i| {
                ...
                const q_row = q_base + row;
                const score = flash_h64_causal_scores_shared[row * config.flash_block_n + col];
                const weight = if (key_row < seq_len and q_row < seq_len and key_row <= q_row)
            ...
            flash_h64_causal_reduce_shared[thread_id] = partial_sum;
        }
        ...
        if (thread_id < config.flash_block_m) {
            const row = thread_id;
            const old_l = flash_h64_causal_row_sum_shared[row];
            const old_scale = flash_h64_causal_old_scale_shared[row];
            const tile_sum = flash_h64_causal_reduce_shared[row * 4] + flash_h64_causal_reduce_shared[row * 4 + 1] + flash_h64_causal_reduce_shared[row * 4 + 2] + flash_h64_causal_reduce_shared[row * 4 + 3];

            flash_h64_causal_row_sum_shared[row] = old_l * old_scale + tile_sum;
        }
    ...
    if (warp_id == 0) {
        inline for (0..4) |acc_i| {
            ...
            const col = coord[1];
            const q_row = q_base + row;
            const denom = flash_h64_causal_row_sum_shared[row];
