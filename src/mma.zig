...
pub const m16n8k16_bf16_f32 = Descriptor{
    .shape = .{ .m = 16, .n = 8, .k = 16 },
    .a_layout = .row,
    .b_layout = .col,
    .a_type = .bf16,
...
pub fn Backend(comptime desc: Descriptor) type {
    ...
    if (sameDescriptor(desc, m16n8k16_bf16_f32)) return M16N8K16BF16F32;
...
pub const M16N8K16BF16F32 = struct {
    pub const desc = m16n8k16_bf16_f32;
    pub const m = 16;
    pub const n = 8;
    pub const k = 16;
    pub const lanes = 32;
    ...
    pub fn loadA(ptr: [*]addrspace(.shared) const u16, stride: usize, lane_id: usize) AFragment {
        const tile_row = (lane_id & 7) + if ((lane_id & 8) != 0) @as(usize, 8) else 0;
        const tile_col = if ((lane_id & 16) != 0) @as(usize, 8) else 0;
        return ldmatrixX4_u16(ptr + tile_row * stride + tile_col);
    }

    pub fn loadB(ptr: [*]addrspace(.shared) const u16, stride: usize, lane_id: usize) BFragment {
        const tile_row = (lane_id & 7) + if ((lane_id & 8) != 0) @as(usize, 8) else 0;
        return ldmatrixX2Trans_u16(ptr + tile_row * stride);
    }
...
fn ldmatrixX4_u16(ptr: [*]addrspace(.shared) const u16) [4]u32 {
    const addr = @intFromPtr(ptr);
    var d0: u32 = undefined;
    var d1: u32 = undefined;
    var d2: u32 = undefined;
...
fn ldmatrixX2Trans_u16(ptr: [*]addrspace(.shared) const u16) [2]u32 {
    const addr = @intFromPtr(ptr);
    var d0: u32 = undefined;
    var d1: u32 = undefined;
    asm volatile (
